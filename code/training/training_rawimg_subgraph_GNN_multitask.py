...
class GraphNN(nn.Module):
    def __init__(self, cell_dim=80, gene_dim=250):
        ...
        self.cellconv = GATv2Conv(in_channels=512, out_channels=int(self.embed_dim/self.head), heads=self.head)
        self.cellnorm = LayerNorm(in_channels=self.embed_dim)
        self.cellhead = Mlp(in_features=self.embed_dim, hidden_features=512*2, out_features=cell_dim)
        
        self.geneconv = GATv2Conv(in_channels=512, out_channels=int(self.embed_dim/self.head), heads=self.head)
        self.genenorm = LayerNorm(in_channels=self.embed_dim)
        self.genehead = Mlp(in_features=self.embed_dim, hidden_features=512*2, out_features=gene_dim)
    ...
    def forward(self, x, edge_index):
        ...
        x_cell = self.cellconv(x=x_spot, edge_index=edge_index)
        x_cell = self.cellnorm(x_cell)
        cell_prediction = self.cellhead(x_cell)
        
        x_gene = self.geneconv(x=x_spot, edge_index=edge_index)
        x_gene = self.genenorm(x_gene)
        gene_prediction = self.genehead(x_gene)
        
        return gene_prediction, cell_prediction


model = GraphNN(cell_dim=args.celltype_num)
...
for epoch in range(num_epochs):
    ...
    if len(train_gene_pred_array[-1].shape) == 1:
        train_gene_pred_array[-1] = np.expand_dims(train_gene_pred_array[-1], axis=0)
    train_gene_pred_array = np.concatenate(train_gene_pred_array)
    if len(train_gene_label_array[-1].shape) == 1:
        train_gene_label_array[-1] = np.expand_dims(train_gene_label_array[-1], axis=0)
    ...
    if len(test_gene_pred_array[-1].shape) == 1:
        test_gene_pred_array[-1] = np.expand_dims(test_gene_pred_array[-1], axis=0)
    test_gene_pred_array = np.concatenate(test_gene_pred_array)
    if len(test_gene_label_array[-1].shape) == 1:
        test_gene_label_array[-1] = np.expand_dims(test_gene_label_array[-1], axis=0)
