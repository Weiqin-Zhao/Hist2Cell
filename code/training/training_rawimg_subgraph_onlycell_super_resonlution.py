...
for epoch in range(num_epochs):
    ...
    for graph in train_loader:
        ...
        label_mask = center_cell_label.sum(dim=1) >= 0
        label_mask_index = label_mask.nonzero().squeeze(1)
        center_cell_pred = torch.index_select(center_cell_pred, dim=0, index=label_mask_index)
        center_cell_label = torch.index_select(center_cell_label, dim=0, index=label_mask_index)
    ...
    for i in range(len(train_cell_pred_array)):
        if len(train_cell_pred_array[i].shape) == 1:
            train_cell_pred_array[i] = np.expand_dims(train_cell_pred_array[i], axis=0)
    train_cell_pred_array = np.concatenate(train_cell_pred_array)
    for i in range(len(train_cell_label_array)):
        if len(train_cell_label_array[i].shape) == 1:
            train_cell_label_array[i] = np.expand_dims(train_cell_label_array[i], axis=0)
    ...
    for i in range(len(test_cell_pred_array)):
        if len(test_cell_pred_array[i].shape) == 1:
            test_cell_pred_array[i] = np.expand_dims(test_cell_pred_array[i], axis=0)
    test_cell_pred_array = np.concatenate(test_cell_pred_array)
    for i in range(len(test_cell_label_array)):
        if len(test_cell_label_array[i].shape) == 1:
            test_cell_label_array[i] = np.expand_dims(test_cell_label_array[i], axis=0)
