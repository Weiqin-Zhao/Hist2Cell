...
for case in test_slides:
    ...
    for slide in pred_and_label:
        ...
        df_label = pd.read_csv(os.path.join("/data1/r20user3/shared_project/Hist2Cell/code/analysis/sptial_colocalization/her2st", case, slide, "MoranR_STNet.csv"), index_col=0)
        df_figure = df_label[['A', 'B', 'R_val']]
        correlation_df = df_figure.pivot(index="A", columns="B", values="R_val")
        for var in correlation_df.index:
            correlation_df.at[var, var] = 1
        ...
        plt.savefig(os.path.join("/data1/r20user3/shared_project/Hist2Cell/code/analysis/sptial_colocalization/her2st", case, slide, "MoranR_clustermap_STNet.png"))
        plt.close()